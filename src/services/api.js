...
export async function fetchModelDrivers() {
  const res = await fetch('/api/model/drivers');
...
export async function uploadCsvDataset(csvContent, fileName) {
  ...
  if (!res.ok) {
    ...
    throw new Error(errData.error || 'Failed to upload CSV dataset');
  }
  return res.json();
}
...
export async function updateRiskThresholds(lowMax, mediumMax) {
  ...
  if (!res.ok) {
    const errData = await res.json().catch(() => ({}));
    throw new Error(errData.error || 'Failed to update thresholds');
  }
  return res.json();
}
