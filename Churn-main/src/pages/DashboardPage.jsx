...
export const DashboardPage = ({ onSelectMember, onNavigateTab, onDatasetNameChange }) => {
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {resetNotice && (
        <div className={`p-4 rounded-xl border text-xs flex items-center justify-between gap-3 ${resetNotice.type === 'success' ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800'
          }`}>
              ...
              {recentMembers.slice(0, 7).map((m) => (
                <tr key={m.Member_ID} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="py-3 px-4 text-center">
                    <span className={`px-2.5 py-1 rounded-full text-[11px] font-mono font-extrabold ${m.riskLevel === 'HIGH' ? 'bg-red-100 text-red-800' :
